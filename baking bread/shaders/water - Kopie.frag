...
{
  return normalize( texture2D(wavemap, coord.s + worldTime / time).xzy - vec3(0.5));
}
...
void main()
{
  ...
  vec2 texCoord = gl_TexCoord[0].st / 50;
  ...
  N = normalize(N);
  N = Normal;//normalize( mix( Normal, N, 0.4));
  ...
  //gl_FragColor.rgba = vec4(C,1) + ( vec4( vec3(foam), 1) * texture2D(foammap,gl_TexCoord[0].st ));//normalize(worldPos);
  gl_FragColor.rgba = vec4( vec3( dot( Normal, sunDir )),1);
  //gl_FragColor.a = 1;
}
