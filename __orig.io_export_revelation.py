...
def ExportMesh(Config, Object):
    ...
    if ( Config.ExportBinaryData ):
        Config.objfile = open(Config.FilePath + Object.name + ".revobj", "wb")
...
#        for Vertex in [Mesh.vertices[Vertex] for Vertex in Vertices]:
        Position = Vertex.co            
        if Config.ExportBinaryData:
            bin = struct.pack( "fff", Position[0], Position[1], Position[2])
...
def WriteMeshUVs(Config, Mesh):
    ...
    for uv in Mesh.uv_layers:
        #Config.File.write("{}UV:{};\n".format("  " * Config.Whitespace, uv.name))    
        for uvloop in uv.data:
            noKey = veckey2d(uvloop.uv)
            if noKey not in globalUVs:
               globalUVs[noKey] = totno
               totno += 1  
               Config.File.write("  " * Config.Whitespace + '%.9f, %.9f;\n' % noKey)
    ...
    for Polygon in Mesh.polygons:    
        ...
        for Vertex in Polygon.vertices:
            ...
            if Polygon.use_smooth:
              s += "{}/{}, ".format(Vertex, globalNormals[veckey3d(Mesh.vertices[Vertex].normal)])  
            else:
              s += "{}/{}/{}, ".format(Vertex, globalNormals[veckey3d(Polygon.normal)],uv_s[:-1])
            v_idx += 1
