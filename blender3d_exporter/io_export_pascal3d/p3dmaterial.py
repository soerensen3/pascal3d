...
class P3DMaterial( p3ddatablock.P3DDataBlock ):
    ...
    def __init__( self, block, root = None, path='', obj = None ):
        super().__init__( block, root, p3dexporthelper.indexedprop.format( 'Materials', block.name ))
        ...
        if ( block.use_transparency ):
          if ( block.transparency_method == 'Mask' ):
            ...
            self.TransparencyMode = "p3dmtStep"
          else:
            self.TransparencyMode = "p3dmMultiply"
        else:
          self.TransparencyMode = "p3dmtNone"
