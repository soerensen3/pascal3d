...
//      s, t: Single;
  ...
  TOcean = class
    private
      ...
      vertex, normal, texture, light_position, _foam,
...
//      void release();
      ...
      procedure render( t: Real; light_pos: TVector; Proj, View, Model: TMatrix4f; Campos: TVector );

      property Paused: Boolean read FPaused write FPaused;
      property Choppiness: Single read FChoppiness write FChoppiness default 1.0;
      property FoamFactor: Single read FFoamFactor write FFoamFactor default 0.99;
      property FoamAmount: Single read FFoamAmount write FFoamAmount default 0.001;
      property FoamThreshh: Single read FFoamThreshh write FFoamThreshh default 4.0;
      property wind: TVector read _w write _w;
  end;
...
begin
  ...
  _foam:= glGetAttribLocation(glProgram, 'foam');
...
var
   ...
   procedure Process( idx1, idxr: Integer; N: TVector );
   begin
     vertices[ idx1 ].y:= h_tilde[ idxr ].im;

     vertices[ idx1 ].x:= vertices[ idx1 ].ox + h_tilde_dx[ idxr ].im * Choppiness;
     vertices[ idx1 ].z:= vertices[ idx1 ].oz + h_tilde_dz[ idxr ].im * Choppiness;
     foam:= Abs( h_tilde_dx[ idxr ].im * Choppiness ) + Abs( h_tilde_dz[ idxr ].im * Choppiness );
     if ( foam > foamthreshh ) then
       vertices[ idx1 ].foam+= foam * foamamount;

     vertices[ idx1 ].nx:=  n.X;
     vertices[ idx1 ].ny:=  n.Y;
     vertices[ idx1 ].nz:=  n.Z;
   end;
  ...
  for m_prime:= 0 to _N - 1 do
      for n_prime:= 0 to _N - 1 do
        begin
          ...
          vertices[ index1 ].foam *= foamfactor;
...
{          vertices[ index1 ].X:= vertices[ index1 ].oX + h_tilde_dx[ index ].im * Choppiness;
          vertices[ index1 ].Z:= vertices[ index1 ].oZ + h_tilde_dz[ index ].im * Choppiness;
          foam:= Abs( h_tilde_dx[ index ].im * Choppiness ) + Abs( h_tilde_dz[ index ].im * Choppiness );
          if ( foam > foamthreshh ) then
            vertices[ index1 ].foam+= foam * foamamount;}
...
{              vertices[ index1 + _N + Nplus1 * _N ].Y:= h_tilde[ index ].im;
              ...
              vertices[ index1 + _N + Nplus1 * _N ].X:= vertices[ index1 + _N + Nplus1 * _N ].oX + h_tilde_dx[ index ].im * Choppiness;
              vertices[ index1 + _N + Nplus1 * _N ].Z:= vertices[ index1 + _N + Nplus1 * _N ].oZ + h_tilde_dz[ index ].im * Choppiness;
              foam:= Abs( h_tilde_dx[ index ].im * Choppiness ) + Abs( h_tilde_dz[ index ].im * Choppiness );
              if ( foam > foamthreshh ) then
//                vertices[ index1 + _N + Nplus1 * _N ].foam+= foam * foamamount;

              vertices[ index1 + _N + Nplus1 * _N ].nx:= n.X;
              vertices[ index1 + _N + Nplus1 * _N ].ny:= n.Y;
              vertices[ index1 + _N + Nplus1 * _N ].nz:= n.Z;}
              Process( index1 + _N + Nplus1 * _N, index, n );
              vertices[ index1 + _N + Nplus1 * _N ].foam:= vertices[ index1 ].foam;
            end;
          if ( n_prime = 0 ) then
            begin
{              vertices[ index1 + _N ].y:= h_tilde[ index ].im;

              vertices[ index1 + _N ].x:= vertices[ index1 + _N ].ox + h_tilde_dx[ index ].im * Choppiness;
              vertices[ index1 + _N ].z:= vertices[ index1 + _N ].oz + h_tilde_dz[ index ].im * Choppiness;
              foam:= Abs( h_tilde_dx[ index ].im * Choppiness ) + Abs( h_tilde_dz[ index ].im * Choppiness );
              if ( foam > foamthreshh ) then
//                vertices[ index1 + _N ].foam+= foam * foamamount;

              vertices[ index1 + _N ].nx:=  n.X;
              vertices[ index1 + _N ].ny:=  n.Y;
              vertices[ index1 + _N ].nz:=  n.Z;}
              Process( index1 + _N, index, n );
              vertices[ index1 + _N ].foam:= vertices[ index1 ].foam;
            end;
          if ( m_prime = 0 ) then
            begin
{              vertices[ index1 + Nplus1 * _N ].y:= h_tilde[index].im;

              vertices[ index1 + Nplus1 * _N ].x:= vertices[ index1 + Nplus1 * _N ].ox + h_tilde_dx[ index ].im * Choppiness;
              vertices[ index1 + Nplus1 * _N ].z:= vertices[ index1 + Nplus1 * _N ].oz + h_tilde_dz[ index ].im * Choppiness;
              foam:= Abs( h_tilde_dx[ index ].im * Choppiness ) + Abs( h_tilde_dz[ index ].im * Choppiness );
              if ( foam > foamthreshh ) then
//                vertices[ index1 + Nplus1 * _N ].foam+= foam * foamamount;

              vertices[ index1 + Nplus1 * _N ].nx:=  n.X;
              vertices[ index1 + Nplus1 * _N ].ny:=  n.Y;
              vertices[ index1 + Nplus1 * _N ].nz:=  n.Z;}
              Process( index1 + Nplus1 * _N, index, n );
              vertices[ index1 + Nplus1 * _N ].foam:= vertices[ index1 ].foam;
            end;
        end;
end;

procedure TOcean.render(t: Real; light_pos: TVector; Proj, View,
  Model: TMatrix4f; Campos: TVector );
...
//  glVertexAttribPointer(texture, 2, GL_FLOAT, ByteBool( 0 ), sizeof( vertex_ocean ), Pointer( 32 ));
  glEnableVertexAttribArray( _foam );
  glVertexAttribPointer( _foam, 1, GL_FLOAT, ByteBool( 0 ), sizeof( vertex_ocean ), Pointer( 24 ));

{  glVertexPointer( 3, GL_FLOAT, sizeof( vertex_ocean ), Pointer( 0 ));
  glNormalPointer( GL_FLOAT, sizeof( vertex_ocean ), Pointer( 0 ));}

  glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, vbo_indices );
  for j:= -repeater to repeater do
    for i:= -repeater to repeater do
      begin
        xfrac:= Round( Campos.x / _length / s ) * _length;
        zfrac:= Round( Campos.z / _length / s ) * _length;
        Model:= MatrixTranslate( Vector( _length * i - xfrac, 0, _length * -j - zfrac ));
        Model:= Model * MatrixScale( Vector( s, s, s ));
        m:= MatrixToDGL( Model );
        glUniformMatrix4fv( _model, 1, ByteBool( GL_FALSE ), @m );
//        ShaderSetParameter4fv( ActiveShader, 'world', m );
