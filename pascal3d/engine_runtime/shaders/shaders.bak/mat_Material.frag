...
void node_tex_planet(vec3 co, float size, out vec4 color, out float fac)
{
	color = vec4(1.0);
	fac = 1.0;
}
...
uniform vec4 unf25;
const float cons27 = float(1.000000);
const vec4 cons31 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
const vec4 cons37 = vec4(0.800000, 0.800000, 0.800000, 0.000000);
const float cons42 = float(50.000000);
const float cons45 = float(0.500000);
uniform vec3 unf50;
const vec3 cons51 = vec3(1.000000, 1.000000, 1.000000);
const float cons60 = float(1.000000);
...
void main(void)
{
	...
	lamp_falloff_invsquare(unf12, tmp10, tmp14);
	...
	mtex_value_invert(cons27, tmp28);
	mix_mult(tmp28, tmp26, cons31, tmp32);
	mtex_value_invert(tmp28, tmp34);
	shade_madd(vec4(tmp5, 1.0), tmp32, cons37, tmp38);
	shade_cooktorr_spec(tmp2, tmp9, tmp4, cons42, tmp43);
	shade_spec_t(tmp34, cons45, tmp16, tmp43, tmp48);
	shade_add_spec(tmp48, unf50, cons51, tmp52);
	shade_add_clamped(vec4(tmp6, 1.0), vec4(tmp52, 1.0), tmp55);
	shade_add(tmp38, tmp55, tmp58);
	mtex_alpha_to_col(tmp58, cons60, tmp61);
	shade_alpha_opaque(tmp61, tmp63);
	linearrgb_to_srgb(tmp63, tmp65);
