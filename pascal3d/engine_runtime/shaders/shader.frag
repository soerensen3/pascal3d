...
//      color_map = texture( tex0, v_texc0.st );
    ...
    if ( numMaps > 0 )
      normal_map = mix( mix( texture( tex0, v_texc0.st ) * 2.0 - 1.0, 
                             texture( tex0, v_texc0.tq * 0.3 ) * 2.0 - 1.0, 0.1 ),
                             texture( tex0, v_texc0.sq * 0.1 ) * 2.0 - 1.0, 5 );
//    if ( numMaps > 1 )
//      color_map = texture( tex1, v_texc0.st );
    ...
    N = normalize( v_normal + T2V * normal_map.xyz / 15 );//mix( v_normal, T2V * normal_map.xyz, 0.1 );
//    N = v_normal;
...
//      shadowcol+= vec4( step( length( lightvec ) / 50 - 0.001, texture( shadow, parabol ).r ));
      ...
      vec2 parabol = texofset[i];
		  ...
		  parabol -=  lightdir.xy * 0.125 / (abs (lightdir.z) + 1); 
		  float r = (length(lightvec)-0.002)/50.0;
//		  float off = 0.002 / length(lightvec);
      ...
      /*for ( float y = -1.5; y <= 1.5; y+= 1 )
        for ( float x = -1.5; x <= 1.5; x+= 1 )
    		   s+= step( r,  texture2D(shadow, parabol + vec2( x, y ) * 0.004 ).r );
		  s /= 16;*/
