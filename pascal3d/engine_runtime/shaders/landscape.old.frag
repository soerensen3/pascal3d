...
float sunIntensity = 0.5;					// Set the Sun intensity (at least 0.0)
vec3 sunDirection = -vec3(-0.265441,-0.421235,-0.867238);		// Set the sun direction (X,Y,Z)
...
vec3 fogColor2 = vec3(0.2,0.2,0.5);			// Set the ending fog color (R,G,B)
...
void main()
{
	...
	//material 2
	vec3 diffuseColor_mat2 = tex3(0.25,0.0);
	float specLevel_mat2 = tex3(0.25,0.25).x;
	vec3 normalDetails_mat2 = prepareNM(tex3(0.25,0.50));
	vec3 diffuseColorFar_mat2 = tex3Far(0.25,0.0);
	vec3 normalDetailsFar_mat2 = prepareNM(tex3Far(0.25,0.5));

	//material 3
	vec3 diffuseColor_mat3 =  tex3(0.5,0.0);
	float specLevel_mat3 = tex3(0.5,0.25).x;
	vec3 normalDetails_mat3 = prepareNM(tex3(0.5,0.50));
	vec3 diffuseColorFar_mat3 = tex3Far(0.5,0.0);
	vec3 normalDetailsFar_mat3 = prepareNM(tex3Far(0.5,0.5));

	//Material 4
	vec3 diffuseColor_mat4 = tex3(0.75,0.0);
	float specLevel_mat4 = tex3(0.75,0.25).x;
	vec3 normalDetails_mat4 = prepareNM(tex3(0.75,0.50));
	vec3 diffuseColorFar_mat4 = tex3Far(0.75,0.0);
	vec3 normalDetailsFar_mat4 =  prepareNM(tex3Far(0.75,0.5));


	// Stencils maps
	float stencil_mat2 = smoothstep( 0.1, 1, texture2D(heightMap, texcoord.st).a );//smoothstep( 0.25, 0.5, texture2D(heightMap, texcoord.st).a );//texture2D(stencil, texcoordaaw.st).b;
	float stencil_mat3 = 0;//texture2D(stencil, texcoord.st).r;
	float stencil_mat4 = smoothstep( 0.0, 0.1, texture2D(heightMap, texcoord.st).a );//texture2D(stencil, texcoord.st).g;


	//lightmap
	float lightmap = 1;//texture2D(lightmap, texcoord.st).r;
	...
	vec3 normalAllDetails = 
		mix(
			mix(
				mix( normalDetails_mat1 ,normalDetails_mat2, stencil_mat2 ),
 				normalDetails_mat3, stencil_mat3 ),
			normalDetails_mat4, 
			stencil_mat4 );
	float specLevelAll = 
		mix(
			mix(
				mix( specLevel_mat1, specLevel_mat2, stencil_mat2 ),
				specLevel_mat3, stencil_mat3 ),
			specLevel_mat4, stencil_mat4 );
	...
	vec3 diffuseColorAll = 
		mix(
			mix(
				mix( diffuseColor_mat1, diffuseColor_mat2, stencil_mat2 ),
				diffuseColor_mat3, stencil_mat3 ),
			diffuseColor_mat4, 
			stencil_mat4 );
	float specularAll = specIntensity * lightmap * 
		max( 0.0,( 0.8 * pow( dot( reflect(sunDirection, normalFinal), viewDirection), shininess)));// compute specular reflection
	...
	vec3 normalDetailsFar = 
		mix(
			mix(
				mix( normalDetailsFar_mat1, normalDetailsFar_mat2, stencil_mat2 ),
				normalDetailsFar_mat3, stencil_mat3 ),
			normalDetailsFar_mat4,
			stencil_mat4 );
	...
	vec3 diffuseColorFar = 
		mix(
			mix(
				mix( diffuseColorFar_mat1, diffuseColorFar_mat2, stencil_mat2 ),
				diffuseColorFar_mat3, stencil_mat3),
			diffuseColorFar_mat4, 
			stencil_mat4 );
